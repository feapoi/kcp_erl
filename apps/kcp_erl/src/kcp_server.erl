...
handle_cast(recv, Kcp) ->
    try
        {Res, _N, Kcp1} = recv(Kcp),
...
handle_cast({send, Buffer}, Kcp) ->
    try
        {Kcp1, Err} = send(Kcp, Buffer),
        ?IF(Err == 0, ok, io:format("send Err:~p~n", [Err])),
        io:format("send Kcp1:~p~n", [Kcp1]),
        {noreply, Kcp1}
    catch
        throw:_ ->
            {noreply, Kcp}
...
handle_cast({set_host_and_port, Host, Port}, Kcp) ->
    try
        {noreply, Kcp#kcp{host = Host, port = Port}}
    catch
        throw:_ ->
            {noreply, Kcp}
    end;
...
%%        conv = Kcp#kcp.conv
    ...
    case AckOnly of
        true ->
            flush_buffer(Kcp#kcp.conv, Kcp#kcp.host, Kcp#kcp.port,Ptr1), %% flash remain ack segments
        ...
        _ ->
            ...
            {Seg1, Ptr2} =
                case Kcp2#kcp.probe band ?IKCP_ASK_SEND =/= 0 of            %% 如果需要探查，设定命令字
                    true ->
                        Ptr11 = make_space(Kcp#kcp.conv,Kcp#kcp.host,Kcp#kcp.port,?IKCP_OVERHEAD, Ptr1, Mtu),
            ...
            {_Seg2, Ptr3} =
                case Kcp2#kcp.probe band ?IKCP_ASK_TELL =/= 0 of
                    true ->
                        Ptr21 = make_space(Kcp#kcp.conv,Kcp#kcp.host,Kcp#kcp.port,?IKCP_OVERHEAD, Ptr2, Mtu),
            ...
            {Ptr4, Kcp6, MinRto1, Change, LostSegs} = bounds_check_elimination(Kcp5#kcp.snd_buf, Ptr3, Kcp5, Current1, Resent, NewSegsCount, Seg, MinRto, 0, 0),

            %% flash remain segments
            flush_buffer(Kcp#kcp.conv,Kcp#kcp.host,Kcp#kcp.port,Ptr4),
            ...
            {Kcp1, Ptr1, Seg2} =
                case Needsend of
                    true ->
                        ...
                        Ptr11 = make_space(Kcp#kcp.conv,Kcp#kcp.host,Kcp#kcp.port,Need, Ptr, Kcp#kcp.mtu),
...
wnd_unused(Kcp) ->
    L = length(Kcp#kcp.rcv_queue),
    case L < Kcp#kcp.rcv_wnd of
        true ->
            Kcp#kcp.rcv_wnd -  L;
        _ ->
            0
        ...
        _ ->
            ...
            Kcp3 =
                case Kcp2#kcp.nocwnd == 0 of
                    true->
                        case Kcp2#kcp.snd_una > SndUna of
                            true ->
                                    ...
                                    true->
                                        ...
                                        Kcp21 =
                                            case Kcp2#kcp.cwnd < Kcp2#kcp.ssthresh of
                                                ...
                                                _ ->
                                                    %% 可发送的最大数据量=拥塞窗口*最大分片大小
                                                    Kcp211 = ?IF(Kcp2#kcp.incr < Mss, Kcp2#kcp{incr = Mss}, Kcp2),
                                                    Kcp22 = Kcp211#kcp{incr = Kcp211#kcp.incr + (Mss*Mss)/Kcp211#kcp.incr + (Mss / 16)},
...
loop_parse_data_1(_I, [], _Sn, Repeat, InsertIdx) ->
    {Repeat, InsertIdx};
    ...
    Kcp1 =
        case Count > 0 of
            true ->
                ...
                Kcp#kcp{rcv_queue = lists:sublist(RcvQueue, Count + 1, LenRcvQueue - Count)};
            _ -> Kcp
        end,
...
loop_rcv_buf([], Count, Kcp) ->
    {Count, Kcp};
loop_rcv_buf([#segment{sn = Sn} | RcvBuf], Count, #kcp{rcv_nxt = RcvNxt, rcv_queue = RcvQueue, rcv_wnd = RcvWnd} = Kcp) ->
...
%% 合并seg中的内容
merge_fragment([], _, Res, N, Count) ->
    {Res, N, Count};
merge_fragment([#segment{data = SegData, frg = SegFrg} | RcvQueue], Kcp, Res, N, Count) ->
